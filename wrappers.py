...
class action_space_discretizer (gym.Wrapper):
    def __init__(self, env, n, dir='vertival'):
        ...
        if dir == "diagonal":
            ...
            self.action_tuple.append(np.array([0.0, 1.0]))    # 左
            self.action_tuple.append(np.array([1.0, 0.0]))    # 上
            self.action_tuple.append(np.array([0.0, -1.0]))   # 右
            self.action_tuple.append(np.array([-1.0, 0.0]))   # 下

        print(self.action_tuple)
    def step(self, action):
        #assert type(action) == int, "we discretized the actions! please give int between 0 and n**2"
        if action == -1:
            action = np.array([0.0,0.0])
        else:
            action = self.action_tuple[int(action)]
